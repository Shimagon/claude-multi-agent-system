"use client";

import { useState } from "react";
import Link from "next/link";

export default function SignupPage() {
  const [invitationCode, setInvitationCode] = useState("");
  const [nickname, setNickname] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isCodeValid, setIsCodeValid] = useState<boolean | null>(null);

  const handleCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const code = e.target.value.toUpperCase();
    setInvitationCode(code);
    // Simple validation for demo (6 characters)
    if (code.length === 6) {
      setIsCodeValid(true);
    } else if (code.length > 0) {
      setIsCodeValid(false);
    } else {
      setIsCodeValid(null);
    }
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Handle signup logic here
    console.log({ invitationCode, nickname, email, password });
  };

  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col">
      {/* Header */}
      <header className="px-4 py-3 flex items-center">
        <Link
          href="/"
          className="min-h-[48px] min-w-[48px] flex items-center justify-center rounded-full hover:bg-[var(--foreground)]/5 active:scale-95 transition-all"
          aria-label="Go back"
        >
          <svg className="w-6 h-6 text-[var(--foreground)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
      </header>

      {/* Main Content */}
      <main className="flex-1 px-6 py-4">
        {/* Welcome Section */}
        <div className="text-center mb-8">
          <div className="text-5xl mb-4">🎉</div>
          <h1 className="text-2xl font-bold text-[var(--foreground)] mb-2">
            Join the Fun!
          </h1>
          <p className="text-[var(--foreground)]/60">
            Create your account and start playing
          </p>
        </div>

        {/* Signup Form */}
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Invitation Code Field - Highlighted */}
          <div className="bg-gradient-to-r from-[var(--accent-yellow)]/20 to-[var(--primary)]/20 rounded-3xl p-4 border-2 border-dashed border-[var(--primary)]/40">
            <label className="block mb-2">
              <span className="text-sm font-bold text-[var(--primary)] flex items-center gap-2">
                <span className="text-lg">🎫</span>
                Invitation Code
                <span className="text-xs font-normal text-[var(--foreground)]/50">(Required)</span>
              </span>
            </label>
            <div className="relative">
              <input
                type="text"
                value={invitationCode}
                onChange={handleCodeChange}
                placeholder="Enter 6-digit code"
                maxLength={6}
                className={`w-full min-h-[56px] px-4 rounded-2xl text-center text-2xl font-bold tracking-[0.3em] uppercase
                           bg-white border-2 transition-all outline-none
                           ${isCodeValid === true
                             ? "border-[var(--accent-green)] bg-[var(--accent-green)]/10"
                             : isCodeValid === false
                               ? "border-red-400 bg-red-50"
                               : "border-[var(--foreground)]/20 focus:border-[var(--primary)]"
                           }`}
              />
              {isCodeValid === true && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-2xl">
                  ✅
                </div>
              )}
            </div>
            <p className="mt-2 text-xs text-[var(--foreground)]/50 text-center">
              Ask a friend for their invitation code to join!
            </p>
          </div>

          {/* Nickname Field */}
          <div>
            <label className="block mb-2">
              <span className="text-sm font-medium text-[var(--foreground)]/70 flex items-center gap-2">
                <span className="text-lg">😊</span>
                Nickname
              </span>
            </label>
            <input
              type="text"
              value={nickname}
              onChange={(e) => setNickname(e.target.value)}
              placeholder="What should we call you?"
              className="w-full min-h-[56px] px-4 rounded-2xl text-lg
                         bg-white border-2 border-[var(--foreground)]/20
                         focus:border-[var(--secondary)] focus:outline-none transition-colors"
            />
          </div>

          {/* Email Field */}
          <div>
            <label className="block mb-2">
              <span className="text-sm font-medium text-[var(--foreground)]/70 flex items-center gap-2">
                <span className="text-lg">📧</span>
                Email
              </span>
            </label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="your@email.com"
              className="w-full min-h-[56px] px-4 rounded-2xl text-lg
                         bg-white border-2 border-[var(--foreground)]/20
                         focus:border-[var(--secondary)] focus:outline-none transition-colors"
            />
          </div>

          {/* Password Field */}
          <div>
            <label className="block mb-2">
              <span className="text-sm font-medium text-[var(--foreground)]/70 flex items-center gap-2">
                <span className="text-lg">🔒</span>
                Password
              </span>
            </label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="Create a password"
              className="w-full min-h-[56px] px-4 rounded-2xl text-lg
                         bg-white border-2 border-[var(--foreground)]/20
                         focus:border-[var(--secondary)] focus:outline-none transition-colors"
            />
            <p className="mt-1 text-xs text-[var(--foreground)]/40 px-2">
              At least 8 characters
            </p>
          </div>

          {/* Submit Button */}
          <button
            type="submit"
            disabled={!isCodeValid || !nickname || !email || !password}
            className="w-full min-h-[56px] rounded-2xl text-white text-lg font-bold
                       bg-gradient-to-r from-[var(--secondary)] to-[var(--accent-pink)]
                       disabled:opacity-50 disabled:cursor-not-allowed
                       active:scale-[0.98] transition-all duration-200
                       shadow-lg shadow-[var(--secondary)]/30"
          >
            Create Account 🚀
          </button>
        </form>

        {/* Divider */}
        <div className="flex items-center gap-4 my-6">
          <div className="flex-1 h-px bg-[var(--foreground)]/10"></div>
          <span className="text-sm text-[var(--foreground)]/40">or</span>
          <div className="flex-1 h-px bg-[var(--foreground)]/10"></div>
        </div>

        {/* Social Login Options */}
        <div className="space-y-3">
          <button
            type="button"
            className="w-full min-h-[56px] rounded-2xl bg-white border-2 border-[var(--foreground)]/10
                       font-medium flex items-center justify-center gap-3
                       active:bg-[var(--foreground)]/5 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </button>

          <button
            type="button"
            className="w-full min-h-[56px] rounded-2xl bg-black text-white
                       font-medium flex items-center justify-center gap-3
                       active:bg-gray-800 transition-colors"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
            </svg>
            Continue with Apple
          </button>
        </div>

        {/* Login Link */}
        <p className="text-center mt-8 text-[var(--foreground)]/60">
          Already have an account?{" "}
          <Link href="/login" className="text-[var(--secondary)] font-bold">
            Log in
          </Link>
        </p>
      </main>

      {/* Bottom Safe Area */}
      <div className="h-8"></div>
    </div>
  );
}
